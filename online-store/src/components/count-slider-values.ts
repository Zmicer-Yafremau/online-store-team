...
export function countSlider(sliderName: 'price' | 'stock'): sliderValueType {
    ...
    const ARRAY_OF_VALUES = Array.from(SLIDER_SET).sort((a, b) => b - a);
    const SLIDER_SET_ON_PAGE = new Set(JSON.parse(localStorage.cards).map((el: cardType) => el[sliderName]));
    const ARRAY_OF_VALUES_ON_PAGE = (Array.from(SLIDER_SET_ON_PAGE) as number[]).sort((a, b) => b - a);
    ...
    return { max: SLIDER_MAX, left: LEFT_SPAN, right: RIGHT_SPAN };
}
