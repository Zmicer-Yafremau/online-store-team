...
export function displayProduct(arrItem: string[], rows: number, currentPage: number, PRODUCT_ITEMS: HTMLDivElement) {
    ...
    PRODUCT_ITEMS.innerHTML = `${paginatedData.reduce((sum, currentId) => {
        ...
        <button type="button" class="btn btn-outline-dark card__add-button ${paginatedData.indexOf(
            String(item.id) as never
        )}">+</button>
        <span class="quantity"> ${quantityCart} </span>
        <button type="button" class="btn btn-outline-dark card__remove-button ${paginatedData.indexOf(
            String(item.id) as never
        )}">-</button>
        </div>
